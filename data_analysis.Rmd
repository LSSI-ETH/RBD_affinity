---
title: "data analysis for RBD affinity on triple mutation variants.\"
author: "hjm"
date: "2024-04-24"
...
plot_synergestic2 <- function(df_individual_scores) {
  ...
  # Filter data to keep only mutation pairs with fold_change ≤ 1
...
# make sure the mutations in three antibodies are the same
long_data <- combined_data %>%
  select(Mutations, Mean_Affinity_WT_3mut, Antibody) %>%
  drop_na() %>%
  group_by(Mutations) %>%
  filter(n_distinct(Antibody) == 3) %>%  # keep only triples seen in all 3
  ungroup()
...
```{r}
binding_site<-read.csv("bindingsite.csv")
...
# Define custom color gradient (red → beige → blue)
...
# Ensure antibody order is: Imdevimab → Bamlanivimab → DBR3_03
desired_order <- c("Imdevimab", "Bamlanivimab", "DBR3_03")
heatmap_matrix <- heatmap_matrix[, desired_order]
...
```{r}
#Figure2A 3 targets in consistent order
merged_df123 <- bind_rows(df1, df2, df3) %>%
  mutate(Variant = sapply(Variant, normalize_variant)) %>%  # Standardize variant names
  group_by(Antibody, Variant) %>%
  summarise(Mean_Relative_Affinity = mean(Mean_Relative_Affinity, na.rm = TRUE), .groups = "drop") %>%  # Take mean for duplicates
...
df_long <- df_barplot %>%
  ...
  mutate(Mean_Affinity = Mean_Affinity - 1)  # Rescale values from 0-3 → -1.5 to 1.5
...
# Order columns to: Imdevimab → Bamlanivimab → DBR3_03
desired_order <- c("Imdevimab", "Bamlanivimab", "DBR3_03")
heatmap_matrix <- heatmap_matrix[, desired_order]
...
```{r}
#here prepare a csv file of triple_mutation_synergy.csv
mut3_score123<-rbind(mut3_score1,mut3_score2,mut3_score3)
colnames(mut3_score123)
mut3_score123<-mut3_score123[,-2]
write.csv(mut3_score123,file = "triple_mutation_synergy.csv")
```
...
# --- normalize Variant: collapse o_* -> "o" (case-insensitive), keep others as-is
normalize_variant <- function(v) {
  out <- v
  out[grepl("^o(_\\d+)?$", v, ignore.case = TRUE)] <- "o"
  out
}

infer_master <- function(df1, df2, df3, variants_540 = NULL) {
  if (!is.null(variants_540)) {
    master <- c(variants_540, "wt", "o")
  } else {
    pooled <- c(df1$Variant, df2$Variant, df3$Variant)
    pooled <- normalize_variant(pooled)
    master <- unique(c(pooled, "wt", "o"))
    message(sprintf("Inferred %d unique variants incl. wt/o (expected 542 including wt+o).",
                    length(master)))
  }
  # enforce unique and return as character
  unique(as.character(master))
}

# --- summarize one df into 5 bins over the master variant set
prep_counts <- function(df, variant_master) {
  df %>%
    mutate(Variant = normalize_variant(Variant),
           rep_count = dplyr::coalesce(as.integer(Replicates), 0L)) %>%
    group_by(Variant) %>%
    summarise(rep_count = max(rep_count), .groups = "drop") %>%
    right_join(tibble(Variant = variant_master), by = "Variant") %>%
    mutate(rep_count = dplyr::coalesce(rep_count, 0L)) %>%
    # keep only bins 0..4; everything else is ignored per your spec
    mutate(rep_bin = pmin(rep_count, 4L)) %>%
    count(rep_bin, name = "n_variants") %>%
    complete(rep_bin = 0:4, fill = list(n_variants = 0L)) %>%
    mutate(
      category = factor(dplyr::case_when(
        ...
        rep_bin == 1 ~ "1 repeat",
        TRUE         ~ "0 repeats"
      ), levels = c("4 repeats","3 repeats","2 repeats","1 repeat","0 repeats")),
      perc = n_variants / sum(n_variants)
    )
}

# --- pie plot helper
plot_pie <- function(count_df, title = "") {
  ggplot(count_df, aes(x = "", y = n_variants, fill = category)) +
...
# =========================
# Run it for df1 / df2 / df3
# =========================

# 1) Build the master list (replace NULL with your `variants_540` if you have it)
variant_master <- infer_master(df1, df2, df3, variants_540 = NULL)

# Quick sanity check:
if (length(variant_master) != 542) {
  message(sprintf("Note: variant_master has %d entries (ideal is 542 = 540 + wt + o).",
                  length(variant_master)))
  # Optionally stop here:
  # stop("Master variant list size mismatch.")
}

# 2) Summarize each binder
c1 <- prep_counts(df1, variant_master)
c2 <- prep_counts(df2, variant_master)
c3 <- prep_counts(df3, variant_master)

# 3) Titles: use Antibody if single-valued, else fallback
title_or <- function(df, fallback) {
  lab <- unique(na.omit(df$Antibody))
  if (length(lab) == 1) as.character(lab) else fallback
}
p1 <- plot_pie(c1, title_or(df1, "Binder 1"))
p2 <- plot_pie(c2, title_or(df2, "Binder 2"))
p3 <- plot_pie(c3, title_or(df3, "Binder 3"))

# 4) Show side-by-side (or save)
p1 + p2 + p3
ggsave("binder_repeat_pies.pdf", p1 + p2 + p3, width = 12, height = 5)
```
